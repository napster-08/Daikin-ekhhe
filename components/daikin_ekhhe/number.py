    ...
    cv.Schema(
        {
            ...
            cv.Optional(P1_LOW_WAT_PROBE_HYST): number.NUMBER_SCHEMA.extend({
            ...
            cv.Optional(P2_HEAT_ON_DELAY): number.NUMBER_SCHEMA.extend({
                cv.GenerateID(): cv.declare_id(DaikinEkhheNumber),
            ...
            cv.Optional(P7_DEFROST_CYCLE_DELAY): number.NUMBER_SCHEMA.extend({
                cv.GenerateID(): cv.declare_id(DaikinEkhheNumber),
                cv.Optional(CONF_MAX_VALUE, default=90): cv.float_,
            ...
            cv.Optional(P8_DEFR_START_THRES): number.NUMBER_SCHEMA.extend({
                cv.GenerateID(): cv.declare_id(DaikinEkhheNumber),
            ...
            cv.Optional(P9_DEFR_STOP_THRES): number.NUMBER_SCHEMA.extend({
                cv.GenerateID(): cv.declare_id(DaikinEkhheNumber), 
            ...
            cv.Optional(P10_DEFR_MAX_DURATION): number.NUMBER_SCHEMA.extend({
                cv.GenerateID(): cv.declare_id(DaikinEkhheNumber),
            ...
            cv.Optional(P17_HP_START_DELAY_DIG1): number.NUMBER_SCHEMA.extend({
                cv.GenerateID(): cv.declare_id(DaikinEkhheNumber),
            ...
            cv.Optional(P18_LOW_WAT_T_DIG1): number.NUMBER_SCHEMA.extend({
                cv.GenerateID(): cv.declare_id(DaikinEkhheNumber),
                cv.Optional(CONF_MAX_VALUE, default=60): cv.float_,
            ...
            cv.Optional(P19_LOW_WAT_T_HYST): number.NUMBER_SCHEMA.extend({
                cv.GenerateID(): cv.declare_id(DaikinEkhheNumber),
            ...
            cv.Optional(P20_SOL_DRAIN_THRES): number.NUMBER_SCHEMA.extend({
                cv.GenerateID(): cv.declare_id(DaikinEkhheNumber),
            ...
            cv.Optional(P21_LOW_WAT_T_HP_STOP): number.NUMBER_SCHEMA.extend({
                cv.GenerateID(): cv.declare_id(DaikinEkhheNumber),
            ...
            cv.Optional(P22_UP_WAT_T_EH_STOP): number.NUMBER_SCHEMA.extend({
                cv.GenerateID(): cv.declare_id(DaikinEkhheNumber),
            ...
            cv.Optional(P25_UP_WAT_T_OFFSET): number.NUMBER_SCHEMA.extend({
                cv.GenerateID(): cv.declare_id(DaikinEkhheNumber),
            ...
            cv.Optional(P26_LOW_WAT_T_OFFSET): number.NUMBER_SCHEMA.extend({
                cv.GenerateID(): cv.declare_id(DaikinEkhheNumber),
                cv.Optional(CONF_MAX_VALUE, default=25): cv.float_,
                cv.Optional(CONF_MIN_VALUE, default=-25): cv.float_,
                cv.Optional(CONF_STEP, default=1): cv.positive_float,
            ...
            cv.Optional(P27_INLET_T_OFFSET): number.NUMBER_SCHEMA.extend({
                cv.GenerateID(): cv.declare_id(DaikinEkhheNumber),
                cv.Optional(CONF_MAX_VALUE, default=25): cv.float_,
                cv.Optional(CONF_MIN_VALUE, default=-25): cv.float_,
                cv.Optional(CONF_STEP, default=1): cv.positive_float,
            ...
            cv.Optional(P28_DEFR_T_OFFSET): number.NUMBER_SCHEMA.extend({
                cv.GenerateID(): cv.declare_id(DaikinEkhheNumber),
                cv.Optional(CONF_MAX_VALUE, default=25): cv.float_,
                cv.Optional(CONF_MIN_VALUE, default=-25): cv.float_,
                cv.Optional(CONF_STEP, default=1): cv.positive_float,
            ...
            cv.Optional(P30_UP_WAT_T_EH_HYST): number.NUMBER_SCHEMA.extend({
                cv.GenerateID(): cv.declare_id(DaikinEkhheNumber),
                cv.Optional(CONF_MAX_VALUE, default=20): cv.float_,
                cv.Optional(CONF_MIN_VALUE, default=2): cv.float_,
                cv.Optional(CONF_STEP, default=1): cv.positive_float,
            ...
            cv.Optional(P31_HP_PERIOD_AUTO): number.NUMBER_SCHEMA.extend({
                cv.GenerateID(): cv.declare_id(DaikinEkhheNumber),
                cv.Optional(CONF_MAX_VALUE, default=80): cv.float_,
                cv.Optional(CONF_MIN_VALUE, default=10): cv.float_,
                cv.Optional(CONF_STEP, default=1): cv.positive_float,
            ...
            cv.Optional(P32_EH_AUTO_TRES): number.NUMBER_SCHEMA.extend({
                cv.GenerateID(): cv.declare_id(DaikinEkhheNumber),
                cv.Optional(CONF_MAX_VALUE, default=20): cv.float_,
                cv.Optional(CONF_MIN_VALUE, default=2): cv.float_,
                cv.Optional(CONF_STEP, default=1): cv.positive_float,
            ...
            cv.Optional(P34_EEV_SH_PERIOD): number.NUMBER_SCHEMA.extend({
                cv.GenerateID(): cv.declare_id(DaikinEkhheNumber),
                cv.Optional(CONF_MAX_VALUE, default=90): cv.float_,
                cv.Optional(CONF_MIN_VALUE, default=20): cv.float_,
                cv.Optional(CONF_STEP, default=1): cv.positive_float,
            ...
            cv.Optional(P35_EEV_SH_SETPOINT): number.NUMBER_SCHEMA.extend({
                cv.GenerateID(): cv.declare_id(DaikinEkhheNumber),
                cv.Optional(CONF_MAX_VALUE, default=15): cv.float_,
            ...
            cv.Optional(P36_EEV_DSH_SETPOINT): number.NUMBER_SCHEMA.extend({
                cv.GenerateID(): cv.declare_id(DaikinEkhheNumber),
            ...
            cv.Optional(P37_EEV_STEP_DEFR): number.NUMBER_SCHEMA.extend({
                cv.GenerateID(): cv.declare_id(DaikinEkhheNumber),
                cv.Optional(CONF_MAX_VALUE, default=50): cv.float_,
                cv.Optional(CONF_MIN_VALUE, default=5): cv.float_,
                cv.Optional(CONF_STEP, default=1): cv.positive_float,
            }), 
            cv.Optional(P38_EEV_MIN_STEP_AUTO): number.NUMBER_SCHEMA.extend({
                cv.GenerateID(): cv.declare_id(DaikinEkhheNumber),
                cv.Optional(CONF_MAX_VALUE, default=45): cv.float_,
                cv.Optional(CONF_MIN_VALUE, default=3): cv.float_,
                cv.Optional(CONF_STEP, default=1): cv.positive_float,
            }), 
            cv.Optional(P40_EEV_INIT_STEP): number.NUMBER_SCHEMA.extend({
                cv.GenerateID(): cv.declare_id(DaikinEkhheNumber),
                cv.Optional(CONF_MAX_VALUE, default=50): cv.float_,
                cv.Optional(CONF_MIN_VALUE, default=5): cv.float_,
                cv.Optional(CONF_STEP, default=1): cv.positive_float,
            }), 
            cv.Optional(P41_AKP1_THRES): number.NUMBER_SCHEMA.extend({
                cv.GenerateID(): cv.declare_id(DaikinEkhheNumber),
            ...
            cv.Optional(P42_AKP2_THRES): number.NUMBER_SCHEMA.extend({
                cv.GenerateID(): cv.declare_id(DaikinEkhheNumber),
                cv.Optional(CONF_MAX_VALUE, default=10): cv.float_,
                cv.Optional(CONF_MIN_VALUE, default=-10): cv.float_,
                cv.Optional(CONF_STEP, default=1): cv.positive_float,
            ...
            cv.Optional(P43_AKP3_THRES): number.NUMBER_SCHEMA.extend({
                cv.GenerateID(): cv.declare_id(DaikinEkhheNumber),
                cv.Optional(CONF_MAX_VALUE, default=10): cv.float_,
                cv.Optional(CONF_MIN_VALUE, default=-10): cv.float_,
                cv.Optional(CONF_STEP, default=1): cv.positive_float,
            ...
            cv.Optional(P44_EEV_KP1_GAIN): number.NUMBER_SCHEMA.extend({
                cv.GenerateID(): cv.declare_id(DaikinEkhheNumber),
                cv.Optional(CONF_MAX_VALUE, default=10): cv.float_,
                cv.Optional(CONF_MIN_VALUE, default=-10): cv.float_,
                cv.Optional(CONF_STEP, default=1): cv.positive_float,
            }), 
            cv.Optional(P45_EEV_KP2_GAIN): number.NUMBER_SCHEMA.extend({
                cv.GenerateID(): cv.declare_id(DaikinEkhheNumber),
                cv.Optional(CONF_MAX_VALUE, default=10): cv.float_,
                cv.Optional(CONF_MIN_VALUE, default=-10): cv.float_,
                cv.Optional(CONF_STEP, default=1): cv.positive_float,
            }), 
            cv.Optional(P46_EEV_KP3_GAIN): number.NUMBER_SCHEMA.extend({
                cv.GenerateID(): cv.declare_id(DaikinEkhheNumber),
                cv.Optional(CONF_MAX_VALUE, default=10): cv.float_,
                cv.Optional(CONF_MIN_VALUE, default=-10): cv.float_,
                cv.Optional(CONF_STEP, default=1): cv.positive_float,
            }), 
            cv.Optional(P47_MAX_INLET_T_HP): number.NUMBER_SCHEMA.extend({
                cv.GenerateID(): cv.declare_id(DaikinEkhheNumber),
            ...
            cv.Optional(P48_MIN_INLET_T_HP): number.NUMBER_SCHEMA.extend({
                cv.GenerateID(): cv.declare_id(DaikinEkhheNumber),
                cv.Optional(CONF_MAX_VALUE, default=10): cv.float_,
                cv.Optional(CONF_MIN_VALUE, default=-10): cv.float_,
                cv.Optional(CONF_STEP, default=1): cv.positive_float,
            ...
            cv.Optional(P49_EVA_INLET_THRES): number.NUMBER_SCHEMA.extend({
                cv.GenerateID(): cv.declare_id(DaikinEkhheNumber),
            ...
            cv.Optional(P50_ANTIFREEZE_SET): number.NUMBER_SCHEMA.extend({
                cv.GenerateID(): cv.declare_id(DaikinEkhheNumber),
                cv.Optional(CONF_MAX_VALUE, default=15): cv.float_,
                cv.Optional(CONF_MIN_VALUE, default=0): cv.float_,
                cv.Optional(CONF_STEP, default=1): cv.positive_float,
            ...
            cv.Optional(P51_EVA_HIGH_SET): number.NUMBER_SCHEMA.extend({
                cv.GenerateID(): cv.declare_id(DaikinEkhheNumber),
